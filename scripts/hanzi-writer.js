...
var HanziWriter = (function () {
  ...
  */
  //将从HTTP下载json修改为从本地获取
  /*
  const getCharDataUrl = char => `../hanziData/cdn.jsdelivr.net/npm/hanzi-writer-data@2.0.1/${char}.json`;
  const defaultCharDataLoader = (char, onLoad, onError) => {
    const localUrl = getCharDataUrl(char);
    // 使用fetch API从本地文件获取数据
    fetch(localUrl)
      .then(response => {
            if (!response.ok) {
                throw new Error(`该汉字json不在库内! status: ${response.status}`);
            }
...
// 构建本地文件的URL路径
const getLocalCharDataUrl = char => `../hanziDataGithub/hanzi-writer-data/data/${char}.json`;

// 构建CDN文件的URL路径
const getCharDataUrl = char => `https://cdn.jsdelivr.net/npm/hanzi-writer-data@${VERSION}/${char}.json`;

const defaultCharDataLoader = (char, onLoad, onError) => {
    const localUrl = getLocalCharDataUrl(char);
    // 先尝试从本地获取文件
    fetch(localUrl)
      .then(response => {
            if (response.ok) {
            ...
            // 如果本地文件不存在（状态码不是200等表示成功的情况），则尝试从网络CDN获取
            return fetch(getCharDataUrl(char))
              .then(cdnResponse => {
                    if (!cdnResponse.ok) {
                        throw new Error(`无论是本地还是网络CDN，都无法获取该汉字数据! status: ${cdnResponse.status}`);
                    }
